...
| 症状（报错/现象） | 首查原因 | 修复 | 详见 |
|---|---|---|---|
| `Unable to connect to Redis` / `Connection refused` | 地址/端口/网络/防火墙；**Boot 2/3 配置前缀写错**（`spring.redis` vs `spring.data.redis`） | 核对前缀与版本；`redis-cli -h host -p port ping` 先通 | `01-connection.md` §4 |
...
| `Could not get a resource from the pool` / pool exhausted | 借连接排队超时（`max-wait`）或池上限不足；连接被事务/阻塞命令占满 | 调池参数；阻塞命令加超时；确认真需要池 | `02-pool.md` §3/§5 |
...
| `OOM command not allowed when used memory > 'maxmemory'` | 默认 `noeviction` + 内存写满 | 设置 `maxmemory` + 合理策略；查内存去哪了 | `08-server-policy.md` §1 |
| 用户随机掉线（与 Sa-Token 共库） | 淘汰策略把 session 挤掉（`evicted_keys` 非零即实锤） | 分实例 / 容量余量；见共库三方案 | `08-server-policy.md` §2 |
...
| TTL 到期的 key 在 `scan`/`dbsize` 里还在 | 惰性+定期删除机制，物理删除滞后 | 无需处理（不可读即正确语义） | `08-server-policy.md` §3 |
...
5. 集群模式：`cluster.nodes` 填的应是**全部 master**（缺节点时部分 slot 路由失败）
