# 06 · 工具落地（栈基线 + 设计→代码映射 + Mock/API 坑）

> §1 是第 0 步探测后的**栈基线决策表**——版本差异唯一集中地；§2-§3 是编码期查表。§3 只收录"用错不报错但埋雷"的隐蔽型坑（如测试假绿、污染其他测试、掩盖耦合）——查文档 30 秒可得的即时报错型坑不收录。

## §1 栈基线决策表（探测结果 → 依赖与写法基线）

| 探测结果（grep `pom.xml`） | 依赖处理 | 写法基线 |
|---|---|---|
| `spring-boot-starter-test` / `spring-boot-starter-parent` | 已引则什么都不加；未引则补 `spring-boot-starter-test`（`scope=test`，无版本号——BOM 已管，且传递 AssertJ / Hamcrest） | JUnit 5 + Mockito（版本跟 Spring Boot BOM） |
| `junit-jupiter` + `mockito-core` 5.x | 缺则按下方坐标补 | `mockStatic` 直接用（5.x 起默认 inline mock maker，`mockito-core` 即可） |
| `mockito-core` 4.x 且需 `mockStatic` | 额外引入 `mockito-inline`（仅 `mockito-core` 不够） | 其余同上 |
| `mockito-core` < 3.4 | —— | `mockStatic` 不可用 → 重构注入（§3「静态调用」小节）或升 Mockito |
| `junit:junit`（JUnit 4） | 跟随既有，不强升 | 按本节末「JUnit 4 差异表」 |
...
<!-- 断言默认 JUnit 原生，集合内容断言/字段分组断言（同一逻辑组）时才升级 AssertJ（触发条件见 SKILL.md"断言库策略"）：
...
### 静态调用：先重构注入，够不着才 mockStatic

首选把静态调用包进接口注入（与"构造器注入"主张一致）：
...
兜底 `mockStatic`（最后手段，版本门槛与工件见 §1 决策表）：
