...
- **同一对象需要 3 个及以上字段断言** —— JUnit 原生会散落多个 `assertEquals`，失败信息割裂、难定位；AssertJ 的 `extracting(...).containsExactly(...)` 集中描述、可读性明显提升。
- **集合内容断言**（断言元素构成/顺序/提取字段）—— JUnit 原生按索引 `get(i)` 脆弱（顺序变了就挂）且啰嗦；AssertJ 的 `containsExactlyInAnyOrder` / `extracting(...)` 不脆、自动描述差异。

**其余场景（简单值、抛异常、1~2 个字段断言）一律 JUnit 原生，不切换。**

- 升级标准可机械执行（数字段数/判断是否集合），不凭感觉——这是防漂移的关键。
- `spring-boot-starter-test` 传递了 AssertJ 依赖，但传递依赖存在 ≠ 必须使用。Spring 项目升级零成本；非 Spring 项目首次触发升级时按 C-CHECK 引入 `assertj-core`。
...
1. **工具栈**：是否 Spring Boot（决定 `spring-boot-starter-test` 一行 vs 逐库坐标）；JUnit 4 还是 5（决定 `@Before`/`@BeforeEach`、`Assert`/`Assertions` 写法）。
...
- 被测类**内部 `new` 出来的对象不能 mock**——这是"测试看起来过了、实则无效"的头号陷阱。正确做法：重构为构造器/Setter 注入，再 mock 注入进来的依赖。
- Mock 的数量是设计味道：**一个测试 mock 了 5+ 个依赖** → 提示被测类职责过重（违反 SRP），应建议拆分，而非硬测（详见规则表 A 级末条）。
- 静态方法 mock（`Mockito.mockStatic`）是最后手段：优先重构为可注入实例方法；确需 mock 才用，且必须 `try-with-resources` 守卫。

## C-CHECK 询问（仅架构守护缺失时触发）

**触发条件**：用户要求"防止架构腐化/禁止跨层调用/禁止循环依赖"，且项目**无 ArchUnit**。低风险场景（普通单测设计）**永不询问**。
...
- **与 `java-coding-guide-pro` 的关系**：正交互补。guide-pro 的 S 级高风险域（金额/日期/并发/加密）天然是测试设计的重点——其边界值正是 off-by-one 高发区，设计测试时优先覆盖这些域的边界。
