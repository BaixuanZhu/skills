...
| 3 | `FieldStrategy.IGNORED` 废弃 | 3.5.x | `FieldStrategy.IGNORED` | `FieldStrategy.ALWAYS`（见 `02-config.md` §7） |
| 4 | SQL 注入器重构 | 3.5.x | 自定义 `extends SqlInjector` / `ISqlInjector` | `extends DefaultSqlInjector`（仅自定义方法注入场景） |
...
1. 升 `com.baomidou:mybatis-plus-*` 版本到 3.5.x 最新（≥3.5.9，统一勿混版本）。
...
5. 自定义 SQL 注入器：`extends SqlInjector` → `extends DefaultSqlInjector`。
6. 清理依赖树：`mvn dependency:tree` 确认无 3.3.x / 3.4.x `mybatis-plus-extension` 残留（避免类路径迁移导致的 `ClassNotFoundException`）。
