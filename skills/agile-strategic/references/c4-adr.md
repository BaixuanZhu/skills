...
| 时间 | ADR | 变更摘要 | 决策来源 |
|------|-----|----------|----------|
| {YYYY-MM-DD HH:MM} | ADR-NNN | {变更摘要}（**已完成状态不变**） | {用户裁决 / agent 推荐待确认} |

> **变更日志与决策区分区**：单 ADR 章节保持 4 行强制结构（背景/决策/后果不动）；追加注记 / 关联条目口径调整 / 评审类裁决走本段。已完成条目口径修订走 `agile-backlog/references/backlog-rules.md §五`「可注记不可改写」原则，不在本表改写历史 ADR 章节。
...
- 决策: {1-3 句，结论 + 关键约束，决策了什么} 依据: {用户拍板 / agent 推荐待确认} + {关键理由 1 句}
- 后果: + {正面 1-3 bullet} / - {负面 1-3 bullet} / 缓解: {缓解措施}
```

> **依据标注**：`决策` 行末尾的 `依据:` 区分「用户拍板」与「agent 推荐待确认」，禁止把 agent 推荐混成用户已确认；用户坚持与约束矛盾的选择时注明「用户坚持，agent 已质疑（理由：…）」（见 `agile-strategic/SKILL.md §3.1 步骤 4`）。
...
禁止内容清单见 `using-agile/references/gate-protocol.md §二 ④`。写 ADR 时若包含禁止内容 → 立即停下，按清单引导到正确归属。
