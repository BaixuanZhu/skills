...
> **核心原则：多写几行代码不是成本，职责错位才是成本**——为清晰边界多写 20 行显式代码，优于靠隐式约定省 5 行。
...
```
> 单构造器时 Spring 自动注入，无需 `@Autowired`。好处是编译期可见：依赖一眼看全，单测直接 new 传 mock。**依赖个数不构成职责判据**（见 §四 9）。
...
3. **变更原因是否来自多个不同业务方？** 运营改活动规则、财务改对账口径、客服改工单流程都落在同一个类上 → 上帝类。
...
- **注入依赖个数**——复杂业务里一个 Service 正常注入 6~8 个协作者（库存、支付、优惠券、风控、物流），只做本域编排仍是单一职责，拿数字卡会误判。
- **类行数、public 方法数**——只是温度计，阈值随业务复杂度浮动，不能当门禁。
...
```
> 优先级：**领域对象自身**（内聚最高、可单测）> 包内工具类 > 跨包工具类。禁跨域塞 `CommonUtil`（判据同 `11` §常量与字面量）。
...
```
> 判据：**创建开销大 + 有内部状态或池化资源 → 归应用级**，不在业务方法里 `new`；有 `close`/`shutdown` 的交给容器托管。典型对象：**线程池**（`05`）、**HTTP 客户端**（`04`）、**`ObjectMapper`**（`04`）、**连接池/数据源**、**`SecureRandom`**（`08`）、**`DateTimeFormatter`**。
